		...
		success: function(data) {
			...
			for (var i = 0; i < cntPhoto; i++) {
				...
				if (arrImg[i].width < arrImg[i].height) {
					...
					// $('.photo__container:last').append(arrImg[i]);
					// $('.photo__container:last').append('<p class="photo__tags">' + arrImg[i].alt +'</p>');
					// console.log($('.photo__tags:last').width());
					// console.log($('.photo__tags:last').height());

				}
				else {
					$container.append('<div class="photo__container photo__container--w2"></div>');
					// $('.photo__container--w2:last').append(arrImg[i]);
					// $('.photo__container--w2:last').append('<p class="photo__tags">' + arrImg[i].alt +'</p>');
				};
